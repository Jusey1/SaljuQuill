...
public class Cannon extends LivingEntity {
	...
	private ItemStack cannon = new ItemStack(QuillItems.CANNON.get());
	...
	public Component getName() {
		if (this.getMainHandItem().is(QuillItems.COPPER_CANNONBALL.get())) {
	...
	public InteractionResult interactAt(Player player, Vec3 v, InteractionHand hand) {
		...
		if (player.level() instanceof ServerLevel && player == this.getOwner()) {
			if (stack.is(QuillItems.CANNONBALL.get()) || stack.is(QuillItems.COPPER_CANNONBALL.get())) {
	...
	public void baseTick() {
		...
		if (this.level() instanceof ServerLevel lvl) {
			...
			if (this.rotation >= 12) {
				...
				this.yHeadRotO = rot;
				this.yHeadRot = rot;
				lvl.playSound(null, this.blockPosition(), QuillModSounds.CANNON_SPIN.get(), SoundSource.AMBIENT, 0.1F, 1.0F);
			} else if (this.rotation == 6 && !this.isEmpty()) {
				for (LivingEntity target : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(12.76D))) {
					if ((target instanceof Enemy || (this.getOwner().getLastHurtMob() == target && target != this)) && this.isLookingAt(target)) {
						...
						this.playSound(QuillModSounds.CANNON_SHOOT.get(), 1.0F, 1.0F);
	...
	public int getCannonballType() {
		return (this.getMainHandItem().is(QuillItems.COPPER_CANNONBALL.get()) ? 1 : 0);
	}
	...
	public boolean isLookingAt(LivingEntity target) {
		if (target.getBoundingBox().getCenter().y() < target.getY() + 0.5 && target.getY() <= this.getY()) {
			return false;
		} else if (target.getY() > (this.getY() + 1.5) || target.getY() < (this.getY() - 0.5)) {
			return false;
		}
